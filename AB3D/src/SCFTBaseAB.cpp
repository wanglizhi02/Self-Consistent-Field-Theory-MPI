...
{	if (myrank==0)
	...
	MDESolver2Order(&Ranges[0], fieldW[0], frdQ, &it_q);// A

	MDESolver2Order(&Ranges[1], fieldW[1], frdQ, &it_q);// A
	...
	int it_qplus =0;
	MDESolver2Order(&Ranges[1], fieldW[1], bakQ, &it_qplus);// fB
	MDESolver2Order(&Ranges[0], fieldW[0], bakQ, &it_qplus);// BA  
//MDESolver4Adams  MDESolver2OrderExtrapolation  MDESolver2Order
}
